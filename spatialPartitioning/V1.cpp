...
public:
    ...
    XYZ static random_bounce(XYZ normal) {
        ...
        if (result < 0) {
            ...
            return raw + 2 * (projected_point - raw);
        }
        else {
            return raw;
        }
    ...
    XYZ static fast_bounce(XYZ normal) {
        XYZ raw = XYZ::normalize(XYZ(fRand(-1, 1), fRand(-1, 1), fRand(-1, 1)));
        decimal result = XYZ::dot(raw, normal);
        if (result < 0) {
...
struct PackagedRay {
    ...
    bool check_lighting;
    PackagedRay() {};
    PackagedRay(XYZ _position, XYZ _slope, XYZ co, XYZ* out, char bounces, char monte_carlo_bounces, short monte_dif, short monte_shad, bool _check_lighting) :
        position(_position),
        slope(_slope),
        coefficient(co),
        output(out),
...
#define RAY_BLOCK_SIZE 8192*16
//#define RAY_BLOCK_SIZE 64
...
public:
    ...
    vector<vector<XYZ*>> enqueue_camera_slopes(Lens* lens, XYZ position, char bounces) {
        int index = 0;
        ...
        for (auto row : lens->slope_grid) {
            ...
            for (auto slope : row) {
                ...
                auto ray = PackagedRay(position, slope, XYZ(1,1,1), output_link, bounces, 2, 512, 512, true);
                enqueue_ray(ray);
    ...
    //void process_ray(Casting_Diagnostics& stats, XYZ slope, XYZ )
                ...
                if (ray_data.remaining_bounces > 0) {
                    if (ray_data.remaining_monte_carlo > 0) {
                        for (int i = 0; i < bounce_count; i++) {
                            XYZ monte_slope = results.material->fast_bounce(results.normal);
                ...
                if (ray_data.check_lighting) {
                    for (int i = 0; i < shadow_count; i++) {
                        for (const PointLikeLight& PLL : lights) {
                            ...
                            XYZ return_coefficient = ray_data.coefficient * results.material->fast_BRDF_co(results.normal, light_slope, flipped_output);
                            if (XYZ::equals(return_coefficient,XYZ(0, 0, 0))) {
                                continue;
                            }
                            if (shadow_count > 1) {
                                return_coefficient = return_coefficient / (shadow_count)/2;
                            }
                            auto ray = PackagedRay(
...
private:
    ...
    vector<vector<XYZ*>> enqueue_rays() {
        ...
        auto LL_array = RE.enqueue_camera_slopes(camera->lens, camera->focal_position+camera->position, 4);
    ...
    void run_engine(bool verbose) {
        ...
        cout << "Approximate speed: " << intToEng((approx_rays / (millis / 1000.0))) << " rays/s (less accurate with large blocks)";
    }
    void fire_casting_event(bool verbose) {
        ...
        if (verbose) {
            ...
            debug_output += iToFixedLength(diagnostics.blocks_created,4) + "NB::";
            debug_output += iToFixedLength(diagnostics.reflections_cast,6) + "R::";
            ...
            debug_output += "[" + iToFixedLength(RE.tag_count[0],3) + "/" + iToFixedLength(RE.tag_total[0],3) + "]";
...
int main()
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        double fnum = fRand(0, 1);
        total += fnum;
        fnums.push_back(fnum);
        //cout << out<< " " << fnum << endl;
    }
    ...
    for (int i = 0; i < num; i++) {
        double fnum = fnums[i];
        //cout << fnum << endl;
        double dif = abs(fnum - average);
        dif_total += dif;
    }

    cout << "average_dif: " << dif_total / num << endl;

    SceneManager* scene_manager = load_default_scene();
   
    scene_manager->render(1920/16,1080/16);
